    ...
    this.config.twitter.stream(type, params, function(stream) {
        ...
        stream.on('error', function(error) {
            if(error.errorSource) {
                console.error('Twitter '+type+' stream error:', error);
            } else {
                console.error('Other Twitter '+type+' stream error', error);
            } 
...
function startPollingTweets(newTweetCallback) { 
}
...
function streamKeywordTweets(newTweetCallback, reconnectSleepSeconds) {  
    ...
    var params = {}
    var track = this.getTrackedList();
    if(track.length > 0) {
        param.track = track;
    }
    
    if(this.config.search_location.length > 0) {
        param.locations = this.config.search_location;
    }
    
    this.streamTweets('filter', params, function(data) {
             
        // if using location, twitter might return a tweet that doesn't match
        // the search terms, so we must parse client side
        // this also allows for negation
        if(this.isMatchingTweet(data)) {
