...
var TwitterEventParser = require('./twitter-event-emitter');
    ...
    Async.parallel([
        ...
        db.fetchOne(select, function(err, result) {
            if(result) {
                ...
                start(readyCallback, newTweetCallback);
            } else {
...
function start(readyCallback, newTweetCallback) {
    startPollingTweets(newTweetCallback);
    streamWeatherTweets(newTweetCallback);
    streamUserReplies(newTweetCallback);
...
function streamWeatherTweets(newTweetCallback) {
